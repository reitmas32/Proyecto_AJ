...
bool DLL_TraverseThree(DLL* this, DLL* other, bool (*pfun)(Item, Item))
{
    /*Verifica la existencia de las DLL*/
	assert(this);																
	assert(other);																

    /*Creamos una bandera que indica si es posible la accon*/
	bool found=false;															

    /*Creamos un apuntador a nodo temporal*/
	NodePtr tmp=other->first;													

    /*Bucle que recorre la primera DLL hasta que el Nodo se convierta en NULL*/
	for(NodePtr it = this->first; it != NULL; it = it->next, tmp=tmp->next)		
	{
        /*Revisamos que la funcion inyectada llamada regrese un valor Falso*/
		if(pfun(it->data,tmp->data)==false)										
		{
            /*Igualamos el apuntador cursor de la segund DLL al apuntador tmp*/
			other->cursor=tmp;													
			found=true;															
			break;																
		}
